...
app.post("/EstadoExercise", function (req, res) {
  const status = req.body;
  Controller.createEstadoExercise(status, res);
});
...
app.post("/send-email", async function (req, res) {
  ...
  const appName = process.env.APP_NAME || "SmartLearn Notification System"; // Obtén el nombre de la aplicación desde las variables de entorno

  if (!from || !to || !text) {
    return res.status(400).json({ error: "Faltan campos requeridos" });
  }

  await Controller.sendEmail(from, to, text, appName, res);
});
app.post("/send-sms", async function (req, res) {
  ...
  await Controller.sendSMS(res, body, from, to);
});
...
// Ver noticias publicadas por id
app.get("/notices/:id", (req, res) => {
  const noticiaId = req.params.id;
  // Llama a la función viewNotice con el ID de la noticia y la respuesta HTTP
  Controller.viewNotice(noticiaId, res);
});

// Ruta para actualizar el estado de una noticia
app.put("/noticias/:id/estado", (req, res) => {
  Controller.updateNoticeStatus;
});
...
app.put("/noticias/:id", (req, res) => {
  ...
  Controller.updateNoticeApproved(id, aprobada, estado, res);
});

app.put("/noticiasr/:id", (req, res) => {
  const id = req.params.id;
  const aprobada = req.body.aprobada;
  const estado = req.body.estado;
  const comentario = req.body.comentario;
  Controller.updateNoticeRejected(id, aprobada, estado, comentario, res);
});

//Ruta para el PUT de la noticias
app.put("/noticiasn/:id", (req, res) => {
  const notice = req.body;
  const id = req.params.id;
  console.log("notice", notice);
  Controller.updateNewNotice(notice, id, res);
});

// Ruta para cambiar el estado de las noticias caducadas
app.put("/noticias/caducadas", (req, res) => {
  Controller.updateExpiredNotices;
});

// Agrega esta ruta para aceptar o rechazar una noticia
app.put("/notices/:id/estado", (req, res) => {
  const id = req.params.id;
  const { status, observacion } = req.body;
  Controller.acceptOrRejectNotice(id, status, observacion, res);
});
...
app.put("/peoples/:id", function (req, res) {
  ...
  Controller.updatePeople(people, id, res);
});
