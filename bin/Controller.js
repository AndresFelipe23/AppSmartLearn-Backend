...
/*------------- SSITEMA DE NOTIFICACIONES - PARTE 1---------------*/
...
class Controller {
  ...
  /* ESTOS DOS ESTABAN COMENTADOS */

  //   async obtenerDatosEjercicio(req, res) {
  //     try {
  //       const exercise = await Exercise.find();
  //       const datosProcesados = [];

  //       for (const ejercicio of exercise) {
  //         const {
  //           task_status,
  //           deliveryDateInicial,
  //           deliveryDateFinal,
  ...
  //       const estudiantesSeleccionados = await People.find({
  //         course: { $in: datosProcesados.map((dato) => dato.Course) }
  //       });

  //       res.json({ estudiantesSeleccionados });
  //     } catch (error) {
  //       res
  //         .status(500)
  //         .json({
  //           error: "Ocurrió un error al procesar los datos del ejercicio."
  //         });
  //     }
  //   }
  ...
  //       const datosProcesados = exercises.map((ejercicio) => ejercicio.Course);

  //       const estudiantes = await People.find();

  //       const estudiantesSeleccionados = [];
  //       console.log("datos", datosProcesados);
  //       datosProcesados.forEach((idCursoProcesado) => {
  //         estudiantes.forEach((estudiante) => {
  //           estudiante.course.forEach((curso) => {
  //             if (curso._id.toString() === idCursoProcesado.toString()) {
  ...
  async sendEmail(from, to, text, appName, res) {
    const mailOptions = {
      from: `"${appName}" <${from}>`, // Incluye el nombre del remitente personalizado
      to: to,
      subject: "Notification", // Asunto personalizado
      text: text
    };
    ...
    transporter.sendMail(mailOptions, (error, info) => {
      ...
      res
        .status(200)
        .json({ message: "Correo electrónico enviado", info: info.response });
    });
  }
  async sendSMS(res, body, from, to) {
    ...
    client.messages
      ...
      .then((message) => {
        ...
        res.status(200).json({ message: "SMS enviado", sid: message.sid });
      })
      .catch((error) => {
        console.error(error);
  ...
  async acceptOrRejectNotice(id, status, observacion, res) {
    try {
      const updatedNotice = await Notice.findByIdAndUpdate(
        id,
        { estado: status, observacion: observacion },
        { new: true }
      );
  ...
  updateNoticeStatus(id, estado, res) {
    Notice.findByIdAndUpdate(id, { estado }, { new: true })
      .then((updatedNotice) => {
        if (!updatedNotice) {
      ...
      .catch((err) => {
        console.error(err);
        res.status(500).send("Error al actualizar el estado de la noticia");
      });
  }

  updateNoticeApproved(id, aprobada, estado, res) {
    const updateFields = { aprobada: aprobada, estado: estado };

    Notice.findByIdAndUpdate(id, updateFields, { new: true })
      .then((updatedNotice) => {
        console.log("Noticia actualizada:", updatedNotice);
        if (!updatedNotice) {
          res.status(404).send("Noticia no encontrada");
        } else {
          res.send(updatedNotice);
  ...
  async updateNewNotice(notice, id, res) {
    try {
      const {
        abstract,
        autor,
      ...
      // Utiliza findByIdAndUpdate para actualizar el documento por su ID
      const updatedNotice = await Notice.findByIdAndUpdate(
        id,
        {
          $set: {
      ...
      res.send({ message: "notice updated", updatedNotice });
    } catch (err) {
      console.error(err);
  ...
  updateNoticeRejected(id, aprobada, estado, comentario, res) {
    const updateFields = {
      aprobada: aprobada,
      estado: estado,
  ...
  // Cambiar el estado de las noticias caducadas
  updateExpiredNotices(req, res) {
    const fechaCaducidad = new Date();
    fechaCaducidad.setDate(fechaCaducidad.getDate() - 10); // Cambiar el número de días según tu requerimiento

    Notice.updateMany(
      { estado: "publicado", fechaPublicacion: { $lt: fechaCaducidad } },
      { estado: "caducado" }
    )
      .then(() => {
        res.send({ message: "Estados de las noticias caducadas actualizados" });
      })
      .catch((err) => {
        console.error(err);
        res
          .status(500)
          .send("Error al actualizar los estados de las noticias caducadas");
      });
  }
  ...
  async connect() {
    try {
      await mongoose.connect(
        ...
        "mongodb://espitiaa321:1003403445pipe@ac-mus6otn-shard-00-00.xcyul6b.mongodb.net:27017,ac-mus6otn-shard-00-01.xcyul6b.mongodb.net:27017,ac-mus6otn-shard-00-02.xcyul6b.mongodb.net:27017/smartlearn?ssl=true&replicaSet=atlas-4lnyje-shard-0&authSource=admin&retryWrites=true&w=majority",
  ...
  async updatePeople(people, id, res) {
    try {
      const {
        name,
        last_name,
      ...
      res.send({ message: "People updated", updatedPeople });
    } catch (err) {
      console.error(err);
      res.status(500).json({ error: "Error updating People" });
    }
  }
  ...
  async setExerciseEstudiante(exercise, res) {
    ...
    actua = await Exercise.findByIdAndUpdate(
      { _id: id_actividad },
    ...
    if (r1.length == 0) {
      ...
      actua = await Exercise.updateOne(
        { _id: id_actividad, "enviados.curso": curso },
        { $push: { "enviados.$.trabajos": object } }
      );
    }
  ...
  async setCalificacionExcersice(paq, res) {
    ...
    var actua = await Exercise.findByIdAndUpdate(
      { _id: id_actividad },
      { $set: { "enviados.$[perf].trabajos": trabajos } },
      {
  ...
  async getActividadEstudent(id) {
    ...
    for (let asignature of data.asignatures) {
      ...
      await People.findOneAndUpdate(
        { _id: id, "asignatures._id": asignature._id },
        { $set: { "asignatures.$.Actividades": art } }
      );
    }
      ...
      People.find({ courestu: curso }, function (err, estudiantes) {
        ...
        if (estudiantes.length > 0) {
          for (let estu of estudiantes) {
            ...
            People.updateOne(
              { _id: estu._id },
              { $set: { asignatures: estu.asignatures } }
            ).then((rawResponse) => {
  ...
  async setMicroClase(micro, res) {
    MicroClase.create(micro.microclase, async function (err, newMicroClase) {
      ...
      for (const curso of micro.cursos) {
        ...
        ManagerMicroClases.find(
          { id_area: ObjectId(micro.id_area) },
          async function (err, area) {
            if (err) throw err;
            if (area.length == 0 && paq != null) {
              console.log(`curso : ${curso}`);
              let obj = new Object();
              (obj.curso = curso), (obj.clases = []);
              ...
              var r = await ManagerMicroClases.findOneAndUpdate(
                { id_area: ObjectId(micro.id_area), "microclase.curso": curso },
                { $push: { "microclase.$.clases": newMicroClase._id } }
              );

              if (r == null) {
                console.log("crea curso");
                await ManagerMicroClases.updateOne(
                  { id_area: ObjectId(micro.id_area) },
                  { $push: { microclase: obj } }
                );
              }
